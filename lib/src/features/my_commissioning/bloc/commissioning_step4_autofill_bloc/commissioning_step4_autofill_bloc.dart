...
class CommissioningStep4AutoFillBloc
    extends
        Bloc<CommissioningStep4AutoFillEvent, CommissioningStep4AutoFillState> {
  final CommissioningStep4AutofillUsecase usecase;

  CommissioningStep4AutoFillBloc(this.usecase)
    : super(CommissioningStep4AutoFillInitialState()) {
    on<CommissioningStep4AutoFillGetEvent>((event, emit) async {
      emit(CommissioningStep4AutoFillLoadingState());

      final result = await usecase(
        CommissioningStep4AutofillParams(event.commissioning_report_id),
      );
