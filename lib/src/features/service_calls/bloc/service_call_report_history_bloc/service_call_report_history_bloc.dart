...
class ServiceCallReportHistoryBloc
    extends Bloc<ServiceCallReportHistoryEvent, ServiceCallReportHistoryState> {
  final ServiceCallReportHistoryUsecase usecase;

  ServiceCallReportHistoryBloc({required this.usecase})
    : super(ServiceCallReportHistoryInitial()) {
    on<FetchServiceCallReportHistory>((event, emit) async {
      ...
      result.fold(
        (failure) =>
            emit(ServiceCallReportHistoryError(message: failure.message)),
        (response) {
          if (response.success) {
